...
test('click test', async ({ page }) => {
  ...
  await expect(await page.locator('#badButton').getAttribute('class')).toMatch(
    /btn-success/
  );
});
...
test('checkbox and radio', async ({ page }) => {
  ...
  await expect(page.locator('[for=radio-1]')).toHaveClass(
    /ui-checkboxradio-checked/
  );
  await expect(page.locator('[for=checkbox-4]')).toHaveClass(
    /ui-checkboxradio-checked/
  );

  await expect(page.locator('[for=checkbox-nested-3]')).toHaveClass(
    /ui-checkboxradio-checked/
  );
  await expect(page.locator('[for=checkbox-nested-4]')).toHaveClass(
    /ui-checkboxradio-checked/
  );
});
